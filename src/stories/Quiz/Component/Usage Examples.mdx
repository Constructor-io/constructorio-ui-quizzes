...
import { Meta, Controls, ArgTypes, Title, Story, Canvas, Description, Markdown } from '@storybook/addon-docs/blocks';
...
### With summary page
Quiz implementation including summary page
...
### Summary page with title and callback
You can pass a `title` to display a heading on the summary page and an `onSummaryPageLoaded` callback that fires when the summary page is shown.
...
### Custom summary page rendering
If you want full control over the summary page markup, use `renderSummaryPage`. It receives the user's answer inputs, a callback to proceed to results, and a callback to jump back to a specific question.
...
export function YourComponent() {
  const args = {
    ...
    summaryPage: {
      isShown: true,
      renderSummaryPage: ({ answerInputs, onResultsClick, onJumpToQuestion }) => (
        <div>
          <h2>Your Selections</h2>
          {Object.entries(answerInputs).map(([questionId, answer]) => (
            <div key={questionId}>
              <span>{answer.questionTitle}</span>
              <button onClick={() => onJumpToQuestion(Number(questionId))}>Edit</button>
            </div>
          ))}
          <button onClick={onResultsClick}>View Results</button>
        </div>
      ),
    },
  };
...
export function YourComponent() {
  const args = {
    ...
    apiKey: 'key_n4SkMH5PFWLdStQZ',
    resultCardOptions: {
      renderResultCard: (result, getters: { getAddToCartButtonProps, getAddToFavoritesButtonProps, getQuizResultSwatchProps }) => {
        <div>
          ...
          <div className='product-price'>{result.data?.price}</div>
            <div className='product-variations'>
              {result.data?.variations?.map((variation) => (
                <button key={variation.variation_id} {...getQuizResultSwatchProps(variation)} />
              ))}
            </div>
