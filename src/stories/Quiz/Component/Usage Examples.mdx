...
You can pass a custom callback function (`onQuizResultClick`) to run on a result item click (eg: redirect to product page). The following example logs the clicked result `id`:
...
You can pass a custom callback function (`onAddToCartClick`) to run on a result add to cart button click. The following example logs the clicked result `id`:
